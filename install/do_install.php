...
$endpoint = "https://socialstreampro.com/api/install";
$stream_opts = [
    "ssl" => [
        "verify_peer"=>false,
...
if( file_exists( $install_path."database.sql" ) ){
    try {
        ...
        $sql = "INSERT INTO sp_purchases (ids, item_id, is_main, purchase_code, version) VALUES (:ids, :item_id, :is_main, :purchase_code, :version)";
        ...
        if ($stmt->rowCount() === 0) {
            ms(array(
                "status" => "error",
                "message" => "Error: Failed to insert purchase record"
            ));
        }
    } catch (PDOException $e) {
        @unlink($file);
