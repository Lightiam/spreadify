...
class AuthModel extends Model
{
	...
	public function signup(){
		...
		$password = $this->db->escapeString($password);
		$confirm_password = $this->db->escapeString($confirm_password);
		$username = $this->db->escapeString($username);
		$agree_terms = $this->db->escapeString($agree_terms);
		$timezone = $this->db->escapeString($timezone);
        ...
        $expiration_date = 0;
        $permissions = NULL;
        $plan_item = db_get("*", TB_PLANS, ["type" => 1, "status" => true]);
        if(!empty($plan_item)){
        	$plan = $plan_item->id;
	 	...
	 	try {
	 		switch ($social_network) {
	            ...
	            case 'facebook':
			        ...
			        $app_version = get_option('facebook_login_app_version', 'v18.0');
