...
class Account_manager extends \CodeIgniter\Controller
{
    ...
    public function widget( $params = [] ){
        ...
        if(isset($params['wheres']) && is_array($params['wheres'])){
            ...
            $accounts = db_fetch("id,ids,pid,name,pid,category,social_network,avatar,login_type,module", TB_ACCOUNTS, [ "can_post" => 1, "team_id" => $team_id, "status" => true ], "social_network", "ASC");
        }
    ...
    public function widget_multi_select( $params = [] ){
        $team_id = get_team("id");
        $accounts = db_fetch("id,ids,pid,name,pid,category,social_network,avatar", TB_ACCOUNTS, "can_post = 1 AND team_id = '{$team_id}' AND status = true", "social_network", "ASC");
